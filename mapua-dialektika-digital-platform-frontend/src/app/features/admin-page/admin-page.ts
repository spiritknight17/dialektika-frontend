...
@Component({
  ...
  standalone: true,
  imports: [HeaderComponent, CommonModule, ReactiveFormsModule, HttpClientModule, MainLabelComponent, AdminPageComponent, UserFilterModal, UserModal],
...
export class AdminPage implements OnInit {
  ...
  filteredUsers$ = combineLatest([
    ...
    map(([users, searchTerm, filter]) => {
      ...
      return [...result].sort((a, b) => {
        switch (filter) {
          case 'username-ascending': return a.username.localeCompare(b.username);
          case 'username-descending': return b.username.localeCompare(a.username);
          case 'email-ascending': return a.email.localeCompare(b.email);
          case 'email-descending': return b.email.localeCompare(a.email);
          case 'id-lowest': return (a.id ?? 0) - (b.id ?? 0);
          case 'id-highest': return (b.id ?? 0) - (a.id ?? 0);
          case 'role-hierarchy': return this.getRolePriority(a.role) - this.getRolePriority(b.role);
          default: return 0;
        }
      });
    })
  ...
  loadUsers() {
    ...
    this.http.get<any[]>(`${environment.apiUrl}/rest/loadusers`, {
      ...
      next: rows => {
        ...
        this.allUsersSubject.next(active.map(u => ({ id: u.id, firstname: u.firstname, lastname: u.lastname, username: u.username, email: u.email, role: u.role })));
      },
  ...
  addUser(user: UserDto) {
    ...
    this.http.post<any>(`${environment.apiUrl}/rest/adduser?${params.toString()}`, null, {
      headers: token ? { Authorization: `Bearer ${token}` } : undefined,
    }).subscribe({
      next: () => {
  ...
  addPost() {
    ...
    if (this.pendingAttachments.length > 0) {
      this.pendingAttachments.forEach(p => {
        formData.append('file', p.file); 
      });
    }

    this.http.post<any>(`${environment.apiUrl}/rest/posts`, formData, {
      headers: token ? { Authorization: `Bearer ${token}` } : undefined,
    }).subscribe({
      next: (res) => {
        this.toastr.success('Post and attachments uploaded successfully!');
        this.resetPostForm();
  ...
  async uploadPostAttachments(postId: number): Promise<void> {
    if (!postId) return; 

    const token = typeof window !== 'undefined' ? (localStorage.getItem('access_token') || '') : '';
    
    const uploads = this.pendingAttachments.map(async (p) => {
      const formData = new FormData();
      formData.append('file', p.file);

      try {
        await this.http.post<any>(
          `${environment.apiUrl}/rest/posts/attachments/upload?post_id=${postId}`,
          formData,
          { headers: token ? { Authorization: `Bearer ${token}` } : undefined }
        ).toPromise();
      } catch {
        this.toastr.error(`Failed to upload "${p.file.name}"`, '', { timeOut: 3000 });
      }
    });
  ...
  removePendingAttachment(p: PendingAttachment) {
    this.pendingAttachments = this.pendingAttachments.filter((a) => a !== p);
    this.cdr.detectChanges();
  }


  async uploadFiles(postId: number) {
    const token = typeof window !== 'undefined' ? (localStorage.getItem('access_token') || '') : '';
    ...
    const uploads = this.pendingAttachments.map(async (p) => {
      ...
      try {
        ...
        this.toastr.error(`Failed to upload ${p.file.name}`);
      }
    });
  ...
  onFileSelected(event: Event): void {
    ...
    if (input.files) {
      for (let i = 0; i < input.files.length; i++) {
        this.pendingAttachments.push({ file: input.files[i] });
      }
  ...
  editUser(user: UserDto) {
    ...
    this.http.put<any>(`${environment.apiUrl}/rest/edituser?${params.toString()}`, null, {
      headers: token ? { Authorization: `Bearer ${token}` } : undefined,
    }).subscribe({
      next: () => {
  ...
  HardDeleteUser() {
    if (this.selectedUsers.length !== 1) return;
    const selectedUser = this.selectedUsers[0];
    const proceed = typeof window !== 'undefined' ? window.confirm(`Permanently delete ${selectedUser.username}? This action cannot be undone.`) : true;
    if (!proceed) return;
    
    const token = typeof window !== 'undefined' ? (localStorage.getItem('access_token') || '') : '';
    const params = new URLSearchParams({ user_id: selectedUser.id!.toString() });
    
    this.http.delete<any>(`${environment.apiUrl}/rest/deleteuser?${params.toString()}`, {
    headers: token ? { Authorization: `Bearer ${token}` } : undefined,
    }).subscribe({
      next: () => {
  ...
  updatePost() {
    if (this.postForm.invalid || !this.selectedPost?.id) return;
    const token = localStorage.getItem('access_token') || '';
    const params = new URLSearchParams({
      title: this.postForm.get('title')?.value || '',
      description: this.postForm.get('description')?.value || ''
    });
    const url = `${environment.apiUrl}/rest/posts/${this.selectedPost.id}?${params.toString()}`;
    const formData = new FormData();
    if (this.pendingAttachments.length > 0) {
     this.pendingAttachments.forEach(p => formData.append('file', p.file));
    }
  ...
  deletePost(postId: number) {
    ...
    const token = localStorage.getItem('access_token') || ''; 
    this.http.delete(`${environment.apiUrl}/rest/posts/${postId}`, {
      headers: token ? { Authorization: `Bearer ${token}` } : undefined
    }).subscribe({
      next: () => {
